...
main(int argc, char *argv[])
{
  ...
  int f=fork();
   d:	e8 b9 02 00 00       	call   2cb <fork>
  if(f==0){
  12:	85 c0                	test   %eax,%eax
  14:	75 14                	jne    2a <main+0x2a>
      exec(argv[1], argv);
  16:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  1a:	8b 43 04             	mov    0x4(%ebx),%eax
  1d:	89 04 24             	mov    %eax,(%esp)
  20:	e8 e6 02 00 00       	call   30b <exec>
  ...
  25:	e8 a9 02 00 00       	call   2d3 <exit>
    status= waitx( &wtime, &rtime);
  2a:	8d 44 24 2c          	lea    0x2c(%esp),%eax
  2e:	89 44 24 04          	mov    %eax,0x4(%esp)
  32:	8d 44 24 28          	lea    0x28(%esp),%eax
  36:	89 04 24             	mov    %eax,(%esp)
  39:	e8 35 03 00 00       	call   373 <waitx>
    printf(1, "Status %d\nTotal Time=%d\nRun Time=%d\nWait Time=%d\n", status, wtime+rtime, rtime, wtime);
...
printf(int fd, const char *fmt, ...)
{
 ...
 5f0:	89 75 d4             	mov    %esi,-0x2c(%ebp)
          s = "(null)";
 5f3:	bb fa 07 00 00       	mov    $0x7fa,%ebx
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 77c:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 77f:	a3 98 0a 00 00       	mov    %eax,0xa98
      return (void*)(p + 1);
 ...
 78f:	90                   	nop
    base.s.ptr = freep = prevp = &base;
 790:	b8 9c 0a 00 00       	mov    $0xa9c,%eax
 795:	ba 9c 0a 00 00       	mov    $0xa9c,%edx
 79a:	a3 98 0a 00 00       	mov    %eax,0xa98
    base.s.size = 0;
 79f:	31 c9                	xor    %ecx,%ecx
 7a1:	bf 9c 0a 00 00       	mov    $0xa9c,%edi
    base.s.ptr = freep = prevp = &base;
 7a6:	89 15 9c 0a 00 00    	mov    %edx,0xa9c
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 7ac:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 7ae:	89 0d a0 0a 00 00    	mov    %ecx,0xaa0
    if(p->s.size >= nunits){
